...
aifmd_readiness: id, question_key(text), status(yes/partial/no/na), notes(text), updated_at — stores per-question answers for AIFMD II readiness assessment; question_key maps to categories: annex_iv_reporting, audit_trail, compliance_calendar, lmt_framework, delegation_oversight, cost_transparency, depositary_arrangements, loan_origination
`.trim();
...
- aifmd_readiness: AIFMD II readiness self-assessment. Score = (yes*1 + partial*0.5) / applicable questions. Categories map to AIFMD II implementation areas.
