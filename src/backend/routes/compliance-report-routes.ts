 ...
 *   - Risk flags
 */
router.get('/compliance/:fundStructureId', async (req: Request, res: Response): Promise<void> => {
  try {
...
router.get('/compliance/:fundStructureId/pdf', async (req: Request, res: Response): Promise<void> => {
  try {
    const { fundStructureId } = req.params;
    const report = await generateComplianceReport(fundStructureId);
