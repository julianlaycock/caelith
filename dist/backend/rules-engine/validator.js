...
const checkLockup = (ctx) => {
    ...
    const daysSinceAcquisition = Math.floor((executionDate.getTime() - acquiredDate.getTime()) / (1000 * 60 * 60 * 24));
...
const checkTransferWhitelist = (ctx) => {
...
function evaluateCompositeRule(ctx, rule) {
    if (!rule.enabled) {
 ...
 */
export function validateTransfer(ctx) {
    const checks = [];
    const violations = [];
    // Run built-in rules
