...
const VALID_OPERATORS = ['eq', 'neq', 'gt', 'gte', 'lt', 'lte', 'in', 'not_in'];
const VALID_COMPOSITE_OPERATORS = ['AND', 'OR', 'NOT'];
...
// ── Claude API Call ─────────────────────────────────────────
async function callClaude(description, context) {
    ...
    if (context) {
        ...
        if (context.fund_legal_form)
            parts.push(`Fund type: ${context.fund_legal_form}`);
        if (context.fund_domicile)
            parts.push(`Fund domicile: ${context.fund_domicile}`);
        if (context.fund_name)
            parts.push(`Fund name: ${context.fund_name}`);
...
// ── Public API ──────────────────────────────────────────────
export async function compileNaturalLanguageRule(request) {
    // Call Claude
...
//# sourceMappingURL=nl-rule-compiler.js.map