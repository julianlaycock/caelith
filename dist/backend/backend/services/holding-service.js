 ...
 */
import { createHolding as createHoldingRepo, findHoldingsByAsset, findHoldingsByInvestor, getCapTable as getCapTableRepo, getTotalAllocatedUnits, findAssetById, investorExists, createEvent, } from '../repositories/index.js';
/**
 * Allocate units to an investor (initial allocation)
 */
export async function allocateHolding(input) {
    ...
    if (currentAllocated + input.units > asset.total_units) {
        throw new Error(`Allocation would exceed total units. Available: ${asset.total_units - currentAllocated}, Requested: ${input.units}`);
    }
    // Validate units
    if (input.units <= 0) {
 ...
 */
export async function getHoldingsForAsset(assetId) {
    return await findHoldingsByAsset(assetId);
}
/**
 * Get holdings for an investor
 */
export async function getHoldingsForInvestor(investorId) {
