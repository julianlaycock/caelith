 ...
 */
export async function createWebhook(url, eventTypes, userId) {
    const id = randomUUID();
    const secret = randomUUID().replace(/-/g, '');
    const now = new Date().toISOString();
    await execute(`INSERT INTO webhooks (id, url, secret, event_types, active, created_by, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, [id, url, secret, JSON.stringify(eventTypes), true, userId || null, now, now]);
    return {
        id,
        url,
        secret,
...
export async function getDeliveries(webhookId, limit = 20) {
    return query('SELECT * FROM webhook_deliveries WHERE webhook_id = ? ORDER BY created_at DESC LIMIT ?', [webhookId, limit]);
}
/**
...
export async function dispatchEvent(eventType, payload) {
    const webhooks = await query(`SELECT * FROM webhooks WHERE active = ?`, [true]);
    for (const webhook of webhooks) {
        ...
        await execute(`INSERT INTO webhook_deliveries (id, webhook_id, event_type, payload, status, attempts, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?)`, [deliveryId, webhook.id, eventType, body, 'pending', 0, now]);
 ...
 */
async function deliverWebhook(deliveryId, url, body, signature) {
    const now = new Date().toISOString();
    try {
        ...
        await execute(`UPDATE webhook_deliveries SET status = ?, response_code = ?, attempts = attempts + 1, last_attempt_at = ? WHERE id = ?`, [res.ok ? 'success' : 'failed', res.status, now, deliveryId]);
    }
    catch {
        await execute(`UPDATE webhook_deliveries SET status = ?, attempts = attempts + 1, last_attempt_at = ? WHERE id = ?`, ['failed', now, deliveryId]);
    }
}
