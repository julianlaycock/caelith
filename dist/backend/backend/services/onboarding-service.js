 ...
 */
import { createOnboardingRecord, findOnboardingById, findOnboardingByAsset, findOnboardingByInvestor, updateOnboardingRecord, } from '../repositories/onboarding-repository.js';
 ...
 */
export async function applyToFund(investorId, assetId, requestedUnits) {
    try {
 ...
 */
export async function checkEligibility(onboardingId) {
    try {
        ...
        if (asset.fund_structure_id) {
            ...
            if (!fund) {
                ...
                if (fund.status === 'active') {
                    const investorType = (investor.investor_type || 'retail');
                    // Look up eligibility criteria
                    const criteria = await findApplicableCriteria(asset.fund_structure_id, investor.jurisdiction, investorType);
                    if (!criteria) {
                        checks.push({
 ...
 */
export async function reviewApplication(onboardingId, decision, reviewedBy, rejectionReasons) {
    try {
        const onboarding = await findOnboardingById(onboardingId);
        if (!onboarding) {
            return { success: false, error: `Onboarding record not found: ${onboardingId}` };
 ...
 */
export async function allocateUnits(onboardingId) {
    try {
        const onboarding = await findOnboardingById(onboardingId);
        if (!onboarding) {
            return { success: false, error: `Onboarding record not found: ${onboardingId}` };
        ...
        const existingHolding = await findHoldingByInvestorAndAsset(onboarding.investor_id, onboarding.asset_id);
        if (existingHolding) {
            ...
            await updateHoldingByInvestorAndAsset(onboarding.investor_id, onboarding.asset_id, existingHolding.units + onboarding.requested_units);
        }
        else {
            // Create new holding
            await createHolding({
