...
export async function generateCapTablePdf(assetId) {
    const assets = await query('SELECT * FROM assets WHERE id = $1', [assetId]);
    if (assets.length === 0)
    ...
    if (asset.fund_structure_id) {
        const funds = await query('SELECT name, legal_form, domicile, regulatory_framework FROM fund_structures WHERE id = $1', [asset.fund_structure_id]);
        if (funds.length > 0)
            fund = funds[0];
    }
    const capTable = await query(`SELECT
    ...
    ORDER BY h.units DESC`, [assetId, asset.total_units]);
    const utilRows = await query(`SELECT
      COALESCE(SUM(units), 0) as allocated,
      COUNT(*) FILTER (WHERE units > 0) as holders
    FROM holdings WHERE asset_id = $1`, [assetId]);
    ...
    return new Promise((resolve, reject) => {
        ...
        if (fund)
            meta.push(fund.legal_form, fund.domicile);
...
function addFooter(doc, L, W, date, reportId) {
...
//# sourceMappingURL=cap-table-pdf.js.map