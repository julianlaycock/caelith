 ...
 */
import { findInvestorById, findHoldingByInvestorAndAsset, findRuleSetByAsset, updateHoldingByInvestorAndAsset, createHolding, createTransfer, createEvent, } from '../repositories/index.js';
...
export async function executeTransfer(request) {
    try {
        ...
        // Deduct from sender
        await updateHoldingByInvestorAndAsset(request.from_investor_id, request.asset_id, fromHolding.units - request.units);
        // Add to receiver
        const toHolding = await findHoldingByInvestorAndAsset(request.to_investor_id, request.asset_id);
        if (toHolding) {
            await updateHoldingByInvestorAndAsset(request.to_investor_id, request.asset_id, toHolding.units + request.units);
        }
        else {
            await createHolding({
