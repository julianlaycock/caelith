 ...
 */
import { findInvestorById, findHoldingByInvestorAndAsset, findRuleSetByAsset, createTransfer, createEvent, } from '../repositories/index.js';
 ...
 */
async function runEligibilityChecks(asset, toInvestor, investmentAmount) {
    const checks = [];
    let criteria = null;
    ...
    const investorType = (toInvestor.investor_type || 'retail');
    // Look up applicable eligibility criteria
    criteria = await findApplicableCriteria(asset.fund_structure_id, toInvestor.jurisdiction, investorType);
    if (!criteria) {
        checks.push({
 ...
 */
export async function simulateTransfer(request) {
    try {
 ...
 */
export async function executeTransfer(request) {
    try {
        // Build validation context
        const context = await buildValidationContext(request);
        // Run rules engine (existing checks)
        ...
        try {
            ...
            const lockResult = await client.query(`SELECT units FROM holdings WHERE investor_id = $1 AND asset_id = $2 FOR UPDATE`, [request.from_investor_id, request.asset_id]);
            ...
            // Deduct from sender
            await client.query(`UPDATE holdings SET units = $1, updated_at = $2 WHERE investor_id = $3 AND asset_id = $4`, [currentUnits - request.units, now, request.from_investor_id, request.asset_id]);
            // Add to receiver (lock if exists, insert if not)
            const receiverResult = await client.query(`SELECT id, units FROM holdings WHERE investor_id = $1 AND asset_id = $2 FOR UPDATE`, [request.to_investor_id, request.asset_id]);
            if (receiverResult.rows.length > 0) {
                await client.query(`UPDATE holdings SET units = $1, updated_at = $2 WHERE investor_id = $3 AND asset_id = $4`, [receiverResult.rows[0].units + request.units, now, request.to_investor_id, request.asset_id]);
            }
            else {
                const holdingId = randomUUID();
                await client.query(`INSERT INTO holdings (id, investor_id, asset_id, units, acquired_at, created_at, updated_at)
           VALUES ($1, $2, $3, $4, $5, $6, $7)`, [holdingId, request.to_investor_id, request.asset_id, request.units, request.execution_date, now, now]);
            }
 ...
 */
async function buildValidationContext(request) {
