...
import { allocateHolding, getHoldingsForAsset, getHoldingsForInvestor, getCapTable, } from '../services/index.js';
...
router.get('/', async (req, res) => {
    try {
        ...
        if (assetId) {
            const holdings = await getHoldingsForAsset(assetId);
            res.json(holdings);
            return;
        }
        if (investorId) {
            const holdings = await getHoldingsForInvestor(investorId);
            res.json(holdings);
            return;
        }
        res.status(400).json({
 ...
 */
router.get('/cap-table/:assetId/pdf', async (req, res) => {
    try {
