...
export async function createOnboardingRecord(input) {
    ...
    await execute(`INSERT INTO onboarding_records
       (id, investor_id, asset_id, status, requested_units, applied_at, created_at, updated_at)
     VALUES ($1, $2, $3, 'applied', $4, $5, $5, $5)`, [id, input.investor_id, input.asset_id, input.requested_units, now]);
    return {
        id,
 ...
 */
export async function findOnboardingById(id) {
    const results = await query('SELECT * FROM onboarding_records WHERE id = $1', [id]);
    if (results.length === 0)
        return null;
 ...
 */
export async function findOnboardingByAsset(assetId) {
    const results = await query('SELECT * FROM onboarding_records WHERE asset_id = $1 ORDER BY applied_at DESC', [assetId]);
    return results.map(rowToOnboardingRecord);
}
/**
 * Find onboarding records by investor
 */
export async function findOnboardingByInvestor(investorId) {
    const results = await query('SELECT * FROM onboarding_records WHERE investor_id = $1 ORDER BY applied_at DESC', [investorId]);
    return results.map(rowToOnboardingRecord);
}
/**
...
export async function updateOnboardingRecord(id, updates) {
    ...
    const params = [id, updates.status, now];
    let paramIdx = 4;
    ...
    await execute(`UPDATE onboarding_records SET ${setClauses.join(', ')} WHERE id = $1`, params);
    return findOnboardingById(id);
}
/**
 * Map DB row → OnboardingRecord
 */
function rowToOnboardingRecord(row) {
    return {
        id: row.id,
