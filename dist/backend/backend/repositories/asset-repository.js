...
export async function createAsset(input) {
    ...
    await execute(`INSERT INTO assets (id, name, asset_type, total_units, fund_structure_id, unit_price, created_at)
     VALUES (?, ?, ?, ?, ?, ?, ?)`, [id, input.name, input.asset_type, input.total_units, fund_structure_id, unit_price, now]);
    const asset = {
        id,
        name: input.name,
...
export async function findAssetById(id) {
    const results = await query('SELECT * FROM assets WHERE id = ?', [id]);
...
export async function findAllAssets() {
    return await query('SELECT * FROM assets ORDER BY created_at DESC');
}
/**
 * Check if asset exists
 */
export async function assetExists(id) {
    const results = await query('SELECT COUNT(*) as count FROM assets WHERE id = ?', [id]);
    return results.length > 0 && results[0].count > 0;
}
/**
 * Get total allocated units for an asset
 */
export async function getTotalAllocatedUnits(assetId) {
    const results = await query('SELECT SUM(units) as total FROM holdings WHERE asset_id = ?', [assetId]);
...
export async function updateAsset(id, input) {
    ...
    if (input.total_units !== undefined) {
        sets.push('total_units = ?');
        params.push(input.total_units);
    }
    if (sets.length === 0) {
        return findAssetById(id);
    }
    params.push(id);
    await execute(`UPDATE assets SET ${sets.join(', ')} WHERE id = ?`, params);
    return findAssetById(id);
}
