...
export async function createEligibilityCriteria(input) {
    const id = randomUUID();
    const now = new Date().toISOString();
    const result = await query(`INSERT INTO eligibility_criteria
 ...
 */
export async function findApplicableCriteria(fundStructureId, investorJurisdiction, investorType) {
    const result = await query(`SELECT * FROM eligibility_criteria
     ...
     ORDER BY
       ...
       effective_date DESC
     LIMIT 1`, [fundStructureId, investorType, investorJurisdiction]);
 ...
 */
export async function findCriteriaByFundStructure(fundStructureId) {
    const result = await query(`SELECT * FROM eligibility_criteria
     WHERE fund_structure_id = $1
       AND superseded_at IS NULL
     ORDER BY jurisdiction, investor_type`, [fundStructureId]);
    return result.map(rowToEligibilityCriteria);
}
/**
 * Supersede a criteria row (soft-replace with a new effective_date row)
 */
export async function supersedeCriteria(id) {
    await query(`UPDATE eligibility_criteria SET superseded_at = CURRENT_DATE WHERE id = $1`, [id]);
}
function rowToEligibilityCriteria(row) {
    return {
        id: row.id,
...
//# sourceMappingURL=eligibility-criteria-repository.js.map